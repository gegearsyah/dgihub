...
export default function CertificateCard({
  ...
  const statusColors = {
    ACTIVE: 'bg-[#0EB0F9]/20 text-[#0EB0F9] border-[#0EB0F9]/30',
    ...
    <div className="bg-gray-800/50 border border-gray-700 rounded-lg p-6 hover:border-gray-600 transition-colors">
        ...
        {(kkniLevel || aqrfLevel) && (
          <div className="flex items-center text-sm">
            ...
            <div className="flex gap-2">
              {kkniLevel && (
                <span className="px-2 py-1 bg-[#0EB0F9]/20 text-[#0EB0F9] rounded text-xs">
                  KKNI {kkniLevel}
                </span>
              )}
              {aqrfLevel && (
                <span className="px-2 py-1 bg-[#0EB0F9]/20 text-[#0878B3] rounded text-xs">
                  AQRF {aqrfLevel}
                </span>
              )}
            </div>
      ...
      <div className="flex gap-2 pt-4 border-t border-gray-700">
        <Link
          ...
          className="flex-1 text-center px-4 py-2 bg-[#0EB0F9] hover:bg-[#0A9DE6] text-white rounded-lg text-sm font-medium transition-colors"
        >
          Share
        </Link>
        <Link
          href={`/talenta/certificates/${certificateId}`}
